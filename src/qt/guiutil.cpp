...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::KEYM, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("keymakercore-%s.lnk", chain);
}
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the Keymaker Core app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Keymaker Core app to startup item list
