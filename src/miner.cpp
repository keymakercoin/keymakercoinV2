...
void static KeymakerMiner(const CChainParams& chainparams)
{
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("KeymakerMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
...
int GenerateKeymakers(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&KeymakerMiner, boost::cref(chainparams)));
    }
    return(numCores);
}
