...
PACKAGE_VERSION=$($SRC_PATH/src/keymakerd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/keymakerd $DEB_BIN
cp $SRC_PATH/src/keymaker-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/keymaker-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/keymakerd.1 $DEB_MAN
cp $SRC_DOC/man/keymaker-cli.1 $DEB_MAN
cp $SRC_DOC/man/keymaker-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/keymakerd.bash-completion $DEB_CMP/keymakerd
cp $SRC_PATH/contrib/keymaker-cli.bash-completion $DEB_CMP/keymaker-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/keymakerd.1
gzip --best -n $DEB_MAN/keymaker-cli.1
gzip --best -n $DEB_MAN/keymaker-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/keymakerd $DEB_BIN/keymaker-cli
